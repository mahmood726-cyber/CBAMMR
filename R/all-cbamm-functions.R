...
# Internal helper functions for run_cbamm_analysis() --------------------

#' Initialize and validate data for CBAMM analysis
#' @keywords internal
.cbamm_initialize_and_validate <- function(data, config) {
  cat("========================================\nCBAMM v7.0 Analysis\n========================================\n")
  ...
  req <- c("study_id","yi","se","study_type")
  ...
  list(data = data, features = features, validator = v)
}

#' Apply transportability weighting
#' @keywords internal
.cbamm_apply_transportability <- function(data, config, target_population) {
  if (config$use_transport && !is.null(target_population)) {
    ...
    if (all(c("age_mean","female_pct") %in% names(data))) {
      pre_age  <- mean(data$age_mean)
      post_age <- stats::weighted.mean(data$age_mean, data$transport_weights)
...
#' Run core meta-analyses (stratified, pooled, advisor, multiverse)
#' @keywords internal
.cbamm_run_core_analyses <- function(data, config, features) {
  results <- list()
...
#' Run publication bias suite
#' @keywords internal
.cbamm_run_publication_bias_suite <- function(data, config, results) {
  # PET-PEESE
...
#' Run advanced analyses (MV, ML, Bayesian, influence, meta-regression)
#' @keywords internal
.cbamm_run_advanced_analyses <- function(data, config, features, results) {
  ...
  if (isTRUE(config$run_mv) && any(duplicated(data$study_id))) {
    ...
    if (isTRUE(config$exact_cov_logOR) && config$effect_measure=="OR" &&
        all(c("ai","bi","ci","di") %in% names(data))) {
  ...
  if (isTRUE(config$run_meta_regression)) {
    results$meta_regression <- run_meta_regression_ns(data, config)
  }
...
#' @keywords internal
.cbamm_create_plots_and_layout <- function(results, data, config) {
  # Plots
  ...
  if (length(results$plots)) {
    ...
    if (length(static_plots) > 0) {
      ...
      if (requireNamespace("patchwork", quietly = TRUE)) {
         safe_try(print(patchwork::wrap_plots(static_plots) + patchwork::plot_layout(design = layout)),
                  context = "printing patchwork combined layout", warn = FALSE)
      } else {
         message("[plot] patchwork not available, skipping combined layout.")
      }
    } else {
      message("[plot] Only interactive plots generated; skipping static wrap.")
    }
  }
...
#' @keywords internal
.cbamm_export_results <- function(results, data, config) {
  # Manuscript table
  ...
  if (config$export_results) {
    ...
    if (!is.null(results$multiverse)) {
      readr::write_csv(results$multiverse, file.path(config$output_dir, "multiverse_results.csv"))
    }
    if (!is.null(results$summary_table)) {
      readr::write_csv(results$summary_table, file.path(config$output_dir, "cbamm_summary_table.csv"))
    }
...
run_cbamm_analysis <- function(data, target_population = NULL, config = setup_cbamm()) {
  ...
  init <- .cbamm_initialize_and_validate(data, config)
  data <- init$data
  features <- init$features
  v <- init$validator

  # Apply transportability
  data <- .cbamm_apply_transportability(data, config, target_population)

  # Run core analyses
  results <- .cbamm_run_core_analyses(data, config, features)

  # Rare-events if signaled OR forced
  rare_trigger <- config$force_rare_events ||
    (config$effect_measure %in% c("OR","RR") &&
     any(c("ai","bi","ci","di") %in% names(data)) && v$rare_hint)
  if (rare_trigger) results$rare_events <- run_rare_event_models(data, config)

  # Publication bias suite
  results <- .cbamm_run_publication_bias_suite(data, config, results)

  # Advanced analyses
  results <- .cbamm_run_advanced_analyses(data, config, features, results)

  # Plots and layout
  results <- .cbamm_create_plots_and_layout(results, data, config)

  # Export results
  results <- .cbamm_export_results(results, data, config)
