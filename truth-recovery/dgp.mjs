...
export function generate(mu, tau2, k, scenario, rng,
  ...
  // A fresh "new study" true effect from the SAME population N(mu, tau^2) — the
  // target a prediction interval is meant to cover (metafor predict()).
  const newTheta = mu + sd * randn(rng);
  if (scenario === 'none') {
    const se = drawSe(rng, k, seLo, seHi);
    const yi = se.map(s => mu + sd * randn(rng) + s * randn(rng));
    return { yi, vi: se.map(s => s * s), newTheta,
  ...
  return { yi, vi, newTheta, info: { nExamined, k, selFrac: k / Math.max(1, nExamined), degenerate } };
}
